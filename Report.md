...
Our MVP addresses this problem by combining live plant data, live weather data, and user-provided gardening conditions into one recommendation system. The product allows a user to enter their location, current month, garden size, sun exposure, water access, garden type, optional USDA hardiness zone, and a plant query. The system then retrieves current weather and plant information and generates a recommendation about whether the plant is a good fit, how much water it may need, and what care considerations the user should keep in mind.
